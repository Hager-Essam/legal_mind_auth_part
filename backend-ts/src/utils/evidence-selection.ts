...
const tokenize = (value: string): string[] => {
  return normalizeArabicQuery(value)
    .split(" ")
...
const getChunkText = (chunk: LegalChunks): string => {
  const lawNameNorm =
    typeof chunk.law_name_normalized === "string"
      ? chunk.law_name_normalized
      : "";
  const lawCategory =
    typeof chunk.law_category === "string" ? chunk.law_category : "";
...
const getCitationBoost = (chunk: LegalChunks): number => {
  const hasLawName =
    typeof chunk.law_name_normalized === "string" &&
    chunk.law_name_normalized.trim().length > 0;
  const hasArticleNumber =
    typeof chunk.article_number === "string" &&
    chunk.article_number.trim().length > 0;

  return hasLawName && hasArticleNumber
    ? 0.08
    : hasLawName || hasArticleNumber
      ? 0.04
      : 0;
};
...
export const scoreEvidenceChunk = (
  question: string,
  chunk: LegalChunks,
): number => {
  ...
  const overlapScore =
    queryTokens.length === 0 ? 0 : overlapCount / queryTokens.length;
