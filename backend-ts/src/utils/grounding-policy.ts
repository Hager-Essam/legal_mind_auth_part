...
const hasStructuredCitation = (chunk: LegalChunks): boolean => {
  ...
  // Court ruling chunks have appeal_number/judicial_year instead of law_name/article_number
  const hasAppeal =
    typeof chunk.appeal_number === "string" &&
    chunk.appeal_number.trim().length > 0;
  const hasCaseSubject =
    typeof chunk.case_subject === "string" &&
    chunk.case_subject.trim().length > 0;
...
export const evaluateGrounding = (chunks: LegalChunks[]): GroundingDecision => {
  if (chunks.length === 0) {
    return {
      ...
      refusalAnswer:
        "لم يتم العثور على أدلة قانونية كافية للإجابة عن السؤال بشكل موثوق.",
    };
  }
  ...
  if (topScore < 0.35 || citedCount === 0) {
    return {
      shouldGenerate: false,
      refusalAnswer:
        "الأدلة القانونية المسترجعة غير كافية لتقديم إجابة موثوقة ومدعومة بنص صريح. يرجى إعادة صياغة السؤال أو تحديد القانون أو المادة بشكل أدق.",
    };
  }
