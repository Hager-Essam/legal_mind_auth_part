...
const getCitationBoost = (chunk: LegalChunks): number => {
  const hasLawName =
    typeof chunk.law_name_normalized === "string" && chunk.law_name_normalized.trim().length > 0;
...
const getArticleMatchBoost = (question: string, chunk: LegalChunks): number => {
  ...
  return parsed.articleNumbers.some((article) => Number.parseInt(article, 10) === chunkArticle) ? 0.2 : 0;
};
...
export const deduplicateEvidence = (chunks: LegalChunks[]): LegalChunks[] => {
  ...
  for (const chunk of chunks) {
    const dedupeKey =
      chunk.chunk_id.trim().length > 0 ? chunk.chunk_id : normalizeArabicQuery(chunk.content).slice(0, 500);
