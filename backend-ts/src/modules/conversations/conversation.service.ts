...
import type { ConversationStatus, Message } from "./conversation.types";
...
const encodeCursor = (value: unknown): string => Buffer.from(JSON.stringify(value)).toString("base64url");

const decodeCursor = <T>(cursor: string): T => {
  try {
    return JSON.parse(Buffer.from(cursor, "base64url").toString("utf8")) as T;
  } catch {
    throw new HttpError(400, "The pagination cursor is invalid.", undefined, "INVALID_CURSOR");
  }
};
...
export class ConversationService {
  ...
  async list(
    ...
    if (input.cursor) {
      ...
      if (Number.isNaN(cursorDate.getTime()) || typeof cursor.conversationId !== "string") {
        throw new HttpError(400, "The pagination cursor is invalid.", undefined, "INVALID_CURSOR");
      }
      filter.$or = [
  ...
  async get(conversationId: string, owner: ConversationOwner) {
    const conversation = await ConversationModel.findOne(this.ownershipFilter(conversationId, owner));
  ...
  async listMessages(
    ...
    return {
      items,
      nextCursor: hasMore && last ? encodeCursor({ sequence: last.sequence }) : null,
    };
  }
  ...
  async softDelete(conversationId: string, owner: ConversationOwner): Promise<void> {
    const result = await ConversationModel.updateOne(this.ownershipFilter(conversationId, owner), {
