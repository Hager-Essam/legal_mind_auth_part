...
export const createHealthController = (services: AppServices) => {
  return {
    health: async (_req: Request, res: Response) => {
      ...
      res.status(mongo.pingOk ? 200 : 503).json({ status, service: env.appName, environment: env.nodeEnv });
    },
    readiness: async (_req: Request, res: Response) => {
      ...
      res.status(ok ? 200 : 503).json({ status: ok ? "ok" : "degraded", checks: { mongo: mongo.pingOk, provider: provider.llmConfigured } });
    },
  };
};
