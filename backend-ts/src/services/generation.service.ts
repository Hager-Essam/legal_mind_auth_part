...
export class GenerationService {
  ...
  async generateChatAnswer(question: string): Promise<string> {
    ...
    console.log(`[GenerationService] chat request → ${url} model=${provider.llmModelFallback}`);
    ...
    console.log(`[GenerationService] chat response ${response.status}: ${text.slice(0, 300)}`);
  ...
  private async generateChatCompletion(model: string, params: GroundedArabicAnswerParams): Promise<string> {
    ...
    const url = `${provider.baseUrl}/chat/completions`;
    console.log(`[GenerationService] grounded request → ${url} model=${model}`);
...
ملاحظة: عدد المصادر المسترجعة: ${params.evidenceCount}. استخدمها جميعاً إذا كانت ذات صلة.`,
    ...
    const text = await response.text();
    console.log(`[GenerationService] grounded response ${response.status}: ${text.slice(0, 300)}`);
