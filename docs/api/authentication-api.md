...
|---|---|---|---|
| `POST /register` | multipart fields + `lawyerIdDocument` | public; 5/hour | 201 message + user |
...
The single credential file is required and must pass the configured PDF/JPEG/PNG
extension/MIME/size checks. Do not set `Content-Type` manually for `FormData`.

The refresh endpoint prefers the `legalmind_refresh_token` cookie over the JSON
field. The public user omits password, credential path, and token hashes. Access
...
token, validation error, upload error, and rate limiting. Resend and forgot
