...
export interface IBlog extends Document {
  ...
  excerpt?: string;
  coverImage?: string;
  category: 'Criminal Law' | 'Civil Law' | 'Corporate Law' | 'Family Law' | 'Labor Law' | 'Tax Law' | 'Other';
  tags: string[];
...
export interface IBlogModel extends Model<IBlog> {
  ...
  getTrending(limit?: number): Promise<IBlog[]>;
}
...
blogSchema.methods.incrementViews = async function (this: IBlog) {
  this.views += 1;
...
blogSchema.statics.getPopular = function (this: IBlogModel, limit = 5) {
...
blogSchema.statics.getTrending = function (this: IBlogModel, limit = 5) {
